        ...
        with open(pose_file, 'r') as f:
            ...
            for line in lines:
                if 'timestamp' in line:
                    continue
                # pose of helmet in base map frame
                T_map_helmet = np.fromstring(line, dtype=float, sep=',')
                T_map_helmet = T_map_helmet[1:]
                T_map_helmet = T_map_helmet.reshape(3, 4)
                T_map_helmet = np.vstack((T_map_helmet, [0, 0, 0, 1]))
                
                #################### We use identity rotation matrix! ####################
                T_map_helmet[:3,:3] = np.identity(3)
                ##########################################################################
                
                poses.append(T_map_helmet)
