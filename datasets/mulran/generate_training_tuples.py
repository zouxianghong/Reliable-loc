# Training tuples generation for Mulran datasets.
import csv
...
from place_recognition.Minkloc3D_V2.misc.point_clouds import icp
...
# create sequence data, refer to Oxford RobotCar / 3-Inhouse Dataset in PointNetVLAD
def create_seq_data(seq_name: str, scan_type: str, ds: MulranSequences):
    ''' sequence name: TrainDataSejong01_02, TestDataDCC01_02\
        scan_type: same as submap_type in Oxford RobotCar Dataset
    '''
    print('Create Sequence Data just like Oxford RobotCar Dataset in PointNetVLAD')
    seq_dir = os.path.join(ds.dataset_root, seq_name)
    ...
    with open(csv_file, 'w', newline='') as newfile:
        ...
        for anchor_ndx in tqdm.tqdm(range(len(ds))):
            item = ds[anchor_ndx]
            timestamp, anchor_pos, anchor_pc = str(item['ts']), item['position'], item['pc']
            # write to csv: ts, y, x
...
if __name__ == '__main__':
    ...
    # create sequence data like Oxford RobotCar Dataset in PointNetVLAD
    ds = MulranSequences(args.dataset_root, sequences, split='train', min_displacement=args.min_displacement)
    create_seq_data('TrainDataSejong01_02/seq1', f'pointcloud_{args.min_displacement}m', ds)
