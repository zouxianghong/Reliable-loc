# Functions and classes operating on a raw Mulran datasets
...
class MulranSequence(Dataset):
    """
    Dataset returns a point cloud from a train or test split from one sequence from a raw Mulran datasets
    """
    def __init__(self, dataset_root: str, sequence_name: str, split: str, min_displacement: float = 0.2):
        assert os.path.exists(dataset_root), f'Cannot access datasets root: {dataset_root}'
...
class MulranSequences(Dataset):
    """
    Multiple Mulran sequences indexed as a single datasets. Each element is identified by a unique global index.
    """
